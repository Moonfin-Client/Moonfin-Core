...
if [ -v OVERRIDE_APP_NAME ] && [ -n "$OVERRIDE_APP_NAME" ]; then
  export FLUTTER_XCODE_MOONFIN_BUNDLE_DISPLAY_NAME="$OVERRIDE_APP_NAME"
  echo "Overridden display name: $OVERRIDE_APP_NAME"
fi

if [ -v OVERRIDE_DEVELOPMENT_TEAM ] && [ -n "$OVERRIDE_DEVELOPMENT_TEAM" ]; then
  export FLUTTER_XCODE_DEVELOPMENT_TEAM="$OVERRIDE_DEVELOPMENT_TEAM"
  export FLUTTER_XCODE_CODE_SIGN_ENTITLEMENTS=Runner/RunnerSideload.entitlements
  export FLUTTER_XCODE_MOONFIN_BUNDLE_IDENTIFIER="$OVERRIDE_DEVELOPMENT_TEAM.org.moonfin.app"
  echo "Overridden development team: $OVERRIDE_DEVELOPMENT_TEAM"
  echo "Overridden bundle identifier: $OVERRIDE_DEVELOPMENT_TEAM.org.moonfin.app"
fi
