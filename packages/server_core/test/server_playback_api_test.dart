...
void main() {
  group('getPlaybackInfo body-rejection fallback', () {
    test(
      ...
      () async {
          ...
          ..interceptors.add(
            _FakeServer((options, h) {
              ...
              if (data.containsKey('DeviceProfile')) {
                // remux's axum Json<PlaybackInfoQuery> rejects the strict profile.
                h.reject(
                  DioException(
        ...
        // First attempt carried the profile; the retry dropped it but kept the
        // live-stream/playback flags so the open handshake still works.
